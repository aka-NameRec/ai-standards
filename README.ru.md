...
- [Обновление развёртывания](#обновление-развёртывания)
...
- `claude`: раскладывает управляемые slash-command templates под `.claude/commands/`; используйте `init-claude-bridge`, чтобы также создать `CLAUDE.md` import-мост
...
- что делает находку пригодной к сообщению: реальная локация в файле, названное нарушенное правило, утверждение, прочитанное по коду, а не по diff, никаких выдуманных правил и раздувания
- поведение по умолчанию «только отчёт» и узкий класс правок, которые ревьюер может сделать без спроса

Сама форма отчёта задана один раз — заполненным примером в [templates/code-review-report.md](templates/code-review-report.md), который `sync-templates` раскладывает в каждый подключивший проект как `.ai-standards/code-review-report.md`; там же раскладывается деталь политики оформления — `.ai-standards/references/code-review-reporting.md` (issue #22, M4): семантика маркеров, политики секций и правила мульти-репозиторных отчётов, места публикации и повторной отправки загружаются в момент написания отчёта, а не живут в `AGENTS.md`. Форму не пересказывает ни один документ, поэтому изменение формата — правка одного файла.

Поскольку триггер и форма отчёта заданы прямо в собранных инструкциях, adapter в `tooling.agents` не требуется — workflow ведёт себя одинаково в Claude Code, Codex, Cursor и любом другом инструменте, читающем `AGENTS.md`. Он совместим с `review-lenses` без конфликтов — детали в usage-гайде. Гейтованная фичей `standard-code-review` skill упаковывает workflow вместе с полным набором линз и проверкой архитектуры и контрактов за один активатор («стандартный code review»).
...
- семантического поиска по коду в большом или мульти-репозиторном пространстве
- удержания retrieval по коду изолированным от retrieval по документации и оперативного контекста
- избегания устаревших результатов за счёт обновления индекса перед запросом
- возобновления прерванных сборок индекса без пере-embedding неизменённых файлов
...
о дрейфе — развёртывание не может молча уехать с версии, которую оно декларирует.

## Обновление развёртывания

Скилл `update-ai-standards` обслуживает сам ai-standards и развёртывается на всех заявленных агентах в `[tooling].agents` без фиче-гейта. Один запрос — «обнови ai-standards» или "update ai-standards" — приводит развёртывание к последнему релизу: включённые фичи перерендериваются молча, фичи, добавленные после зафиксированной версии, анонсируются по метаданным `[feature_meta]` из `registry.toml` с рекомендацией относительно стеков проекта, и включается только то, что пользователь подтвердил. Русский гид: [docs/standards-update-usage.ru.md](docs/standards-update-usage.ru.md)

### Стандартные промпты установки и обновления

Дайте своему агенту один из этих промптов дословно. Установка работает «с нуля», имея только URL репозитория; обновление — из развёрнутого скилла, либо из URL, когда развёртывание старше скилла.

Установка (в проекте ничего нет):

```text
Установи в этом проекте ai-standards из репозитория https://github.com/aka-NameRec/ai-standards:
склонируй его, прочитай templates/standards-update/update-ai-standards.SKILL.md и выполни его
Install mode — обследуй проект, предложи фичи с рекомендациями, разверни выбранное после моего
подтверждения.
```

Обновление (развёртывание этого или более нового релиза):

```text
Обнови ai-standards до последней версии.
```

Обновление (развёртывание старше скилла — бутстрап):

```text
Обнови ai-standards в этом проекте из репозитория https://github.com/aka-NameRec/ai-standards:
склонируй его, прочитай templates/standards-update/update-ai-standards.SKILL.md и выполни его
Upgrade mode — молча обнови включённые фичи, анонсируй новые, которых ещё нет в манифесте,
включи подтверждённое и выставь пин версии.
```
