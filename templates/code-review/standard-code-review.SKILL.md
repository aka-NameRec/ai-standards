...
2. **Run the `code-review` passes** following `references/procedure.md` (the pass-by-pass procedure this skill carries), with evidence read from the code, not the diff; findings cite file, line, and the violated rule.
...
5. **Apply the small-fix policy** from `references/procedure.md`: safe-by-reading fixes are made and recorded as ✅; refactors, migrations, contracts, and anything needing a test run are reported.
