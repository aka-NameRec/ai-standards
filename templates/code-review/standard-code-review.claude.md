...
4. **Check architecture and contracts.** For every changed module: find its `MODULE_CONTRACT.md` (ownership, non-goals, invariants), the accepted records under `docs/architecture/**`, and the module map when one exists. A violation cites the exact clause. A changed major module with no contract is reported as a `(no contract)` note — a gap, not a defect.
