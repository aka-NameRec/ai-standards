# Module Contract Discovery Gate — Procedure

Full detail of the discovery gate: the commands, the coverage criteria, the
reasoning constraints, the index-entry shape, and the reporting contract. The
always-loaded `Module Contract Discovery Gate` rules carry the mandate, the
canonical sources, and the strict rule; this reference carries the detail.
When this file is present in the project, follow it whenever the gate fires.
