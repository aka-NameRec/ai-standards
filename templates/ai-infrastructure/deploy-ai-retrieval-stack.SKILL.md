---
name: deploy-ai-retrieval-stack
description: Deploy the AI retrieval stack (Basic Memory, Chroma) in a project. Use when the user asks to set up AI infrastructure, deploy Basic Memory/Chroma, or initialize the retrieval stack.
disable-model-invocation: true
---

# Deploy AI Retrieval Stack

This skill deploys the AI retrieval stack in a project: Basic Memory (documentation retrieval, including local working memory) and Chroma (semantic code search). It is capability-based: deploy only the layers enabled in the project manifest (`ai.project.toml` `features`). If the manifest is absent, ask the user which layers to deploy. A structural code-intelligence layer may join the stack later; deploy it only when its feature is explicitly enabled.
...
- When the project enables local working memory, make sure `/docs/local/` is in `.gitignore`: local memory is user-local and is not committed by default.
...
- Wire the MCP servers consistently in every client the project uses.
