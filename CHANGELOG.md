...
### Added
- **Scenario contracts `BM-001`/`BM-002`** (issue #22 stage 3, `STD-CHANGE-0001`; EN/RU, bound to new rules `BM-001`–`BM-003`): BM-001 pins the note shape on create (dated file name, Russian frontmatter `title` repeated as the `# ` heading, closing `Observations`/`Relations`) verified from the fixture file system; BM-002 pins the sync-hygiene trigger after VCS operations, judged from the answer text. Rules for the basic-memory fragment previously had no ids at all.
...
### Changed
- **Merge-gate rework (issue #22 stage 3, lives in `ai-standards-evals`)**: scorer failures are classified deterministically (dishonesty / formatting / substantive); the baseline-vs-candidate comparison downgrades only on dishonesty, majority substantive breakdown, or a new substantive failure class — formatting variance is recorded per epoch but never blocks. Spawned agents run with an isolated XDG config (no operator instructions), and harness timeouts retry once. Rules: `docs/scorer-guidelines.md` in `ai-standards-evals`.
- **M5 basic-memory operations relocation (issue #22 stage 3, `STD-CHANGE-0001`)**: new template `templates/basic-memory-operations.reference.md`, deployed as `.ai-standards/references/basic-memory-operations.md` gated on the `basic-memory` feature. Masking targets, permalink and legacy-flag behavior, rename mechanics, repair-tool semantics, and the reindex matrix now load when operating the tree; the fragment keeps routing, the genre doctrine, a compressed note-shape rule, and the sync triggers. Rendered `Basic Memory Usage` section: 8,833 → 5,815 bytes. New behavioral coverage: BM-001/BM-002 in `ai-standards-evals`.
- **M6 policy compression (issue #22 stage 3, `STD-CHANGE-0001`)**: same-content, fewer-tokens compression of `fragments/tools/project-memory.md` (4,547 → 4,256 bytes) and `fragments/process/structured-artifacts.md` (4,597 → 4,285 bytes); the pinned invariant phrasings are preserved verbatim and guarded by tests.
- **M4 progressive-disclosure relocation (issue #22, change set `STD-CHANGE-0001`)**: new template `templates/code-review-reporting.reference.md`, deployed by `sync-templates` as `.ai-standards/references/code-review-reporting.md` gated on the `code-review` feature. The reporting-policy detail — marker semantics, `What Was Done`/`How It Was Done`/`Verification`/`Dependencies`/`Task` policies, multi-repository reports, posting destination, resend handling — now loads at report time instead of living in `AGENTS.md`. The fragment keeps the trigger, scope, finding invariants, shape pointer, fallback order, the verbatim version-line invariant (the M3 lesson), the chat-language rule, and — added after the first candidate epoch failed the eval gate — a minimal Russian section legend for the fallback path, without which fallback reports degraded into mixed-language forms under a Russian ambient context. The full Russian localization legend moved into the worked example, which the agent reads exactly when the report is written. Self-hosted `AGENTS.md` after stages 1–3: 61,888 → 56,508 bytes (−8.7%); the rendered `Code Review` section: 7,781 → 5,914 bytes. Behavior verified by the `ai-standards-evals` merge gate (stage-3, final): after the gate rework (failure-class taxonomy, environment stabilization) the baseline-vs-candidate comparison returned ACCEPT with zero downgrades — 2 epochs per side in the stabilized environment, LLM judge 20/20. Gate history: the first round caught the version-line salience regression (fixed); the second rejected on formatting-variance classes that motivated the class-aware comparison.
