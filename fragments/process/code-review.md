...
- Keep `What Was Done` and `How It Was Done` as short prose, and omit both only for a standalone review with no implementation work behind it. Name reuse and conventions the change got right there rather than as findings, because a marker has to mean outstanding work for the markers to stay scannable.
