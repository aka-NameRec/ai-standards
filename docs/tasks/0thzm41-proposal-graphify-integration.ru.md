---
title: '0thzm41 — Предложение: интеграция Graphify в ai-standards'
permalink: ai-standards/tasks/0thzm41-proposal-graphify-integration.ru
---
...
`ai-standards` теперь стандартизует политику использования и развёртывание для ConPort, Basic Memory и Chroma (задача `0ta01xp`). Слой структурного анализа кода — оставшийся пробел. Graphify уже используется в крупном backend-коде (CLI `graphify`): `graphify update .` создаёт `graphify-out/graph.json` (~9k узлов / ~23k рёбер) и предоставляет операции `query` / `path` / `explain` над графом структуры кода.
...
- Graphify **не является** фичей по умолчанию. Он нацелен на крупные, слоистые, integration-heavy кодовые базы (крупного масштаба); для небольших и средних проектов это оверхед.
...
├── graphify-out/            # per-repo graph.json (gitignored runtime или repo-local по прецеденту исходного развёртывания)
└── state/                   # per-target graph state (gitignored runtime)
```

Конфигурация graph-таргетов объявляется в существующем `.ai-standards/code-index.toml` в секции `[[graph]]` (по исходному развёртыванию), так что один конфиг управляет и коллекциями Chroma, и таргетами Graphify.
...
- `graphify-out/` — workspace-central (`.ai-standards/`) или repo-local (исходное развёртывание использовало repo-local вывод графа).
