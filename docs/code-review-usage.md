...
## Rules Version In The Report

Every report opens with an `ai-standards <version>` line copied from the `Generated by ai-standards` header of the generated `AGENTS.md` in force. It records which rule set the review ran under, so a report pasted into a pull request stays interpretable after the standard has moved on. When the header is missing, the report says the version is undetermined rather than guessing.
