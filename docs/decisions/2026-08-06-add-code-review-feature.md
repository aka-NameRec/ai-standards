...
Around the example sit four short operating rules: severity markers (🔴 blocking, 🟡 should fix, 🔵 optional, ✅ fixed, with a `→ fixed:` or `→ left as-is:` tail written onto the finding's own line); full localization to the chat language with the wordless markers exempt; chat output by default and a file only on request; and a later "resend the report" reclassifying findings already on record instead of re-reviewing from scratch.
...
- a task reference is never invented; the line is omitted when none is known
