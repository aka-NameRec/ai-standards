...
Навык ручного запуска, распространяемый на четыре среды (Codex, Claude, Kilo, Cursor), разворачивает стек в проекте в порядке, устраняющем гонки, наблюдённые в DevCats:
...
```
.ai-standards/
├── scripts/code_index.py   # managed template (обобщён из DevCats)
...
### Хранить знания о развёртывании только в DevCats `code_index.py`
...
- токен-ловушки DevCats (синхронный polling, per-file upsert, краш манифеста, сброс BM, cloud routing, уход detection, дрифт MCP) закодированы как превентивный порядок в навыке;
...
- шаблон `code_index.py` нужно обобщить из DevCats-специфичной реализации и поддерживать как эталон;
