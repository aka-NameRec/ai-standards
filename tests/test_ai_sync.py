...
def test_render_contains_expected_markers(tmp_path: Path) -> None:
    ...
    assert "<!-- Project version:" not in result.content
...
def test_report_template_sections_stay_in_sync_with_the_fragment() -> None:
    """Guard the places that restate the report shape.

    `templates/code-review-report.md` defines the shape. The fragment restates the
    section names once, in the fallback ordering used when a project never synced
    the template; the localization legend moved into the template itself with M4
    (issue #22). Renaming a section in the template must therefore fail here
    rather than silently diverge from those two lists.
    """
    ...
    # The localization legend lives in the template: it is read exactly when the
    # report is written, and the fragment no longer restates the mapping.
    assert "Localization legend" in template
    for russian_name in (
        "Код-ревью",
        "Что сделано",
        "Как сделано",
        ...
        assert russian_name in template, f"{russian_name!r} is missing from the template legend"
...
def test_code_review_reporting_reference_carries_the_moved_policies() -> None:
    """M4 (issue #22) moved the reporting policy detail out of the always-loaded
    fragment into the deployed reporting reference. The fragment keeps the
    routing pointer, the fallback order, and the version-line invariant; every
    moved policy must remain stated in the reference, or the behavior would be
    loaded nowhere."""
    reference = (
        REPO_ROOT / "templates" / "code-review-reporting.reference.md"
    ).read_text(encoding="utf-8")
    fragment = (REPO_ROOT / "fragments" / "process" / "code-review.md").read_text(
        encoding="utf-8"
    )

    # The routing pointer and the fallback stay always-loaded.
    ...
    for moved in (
        "→ fixed:",
        "→ left as-is:",
        "no verdict and no resolution section",
        "What Was Done",
        "How It Was Done",
        "name what was not checked",
        "Drop `Dependencies` when the change is self-contained",
        "Omit `Task` when no tracked reference is known",
        "tracker_url",
        "one report per",
        "fenced Markdown code block",
        "resend the report",
    ):
        assert moved in reference, f"{moved!r} is missing from the reporting reference"
...
def test_init_project_seeds_current_ai_standards_version(tmp_path: Path) -> None:
    ...
    assert f'ai_standards_version = "{CURRENT_RELEASE_PIN}"' in manifest
    assert 'project_version' not in manifest
...
def test_sync_deploys_standard_review_adapters_when_code_review_enabled(tmp_path: Path) -> None:
    ...
    # code-review report and reporting-reference templates.
    ...
    for result in results:
        ...
        if destination.endswith("code-review-report.md"):
            continue
        if destination.endswith("code-review-reporting.md"):
            assert "reporting policy" in content, result.destination_path
            continue
...
def test_check_fails_when_manifest_pin_drifts_from_the_source(tmp_path: Path) -> None:
    ...
    manifest = (
        MANIFEST_RELEASE_BLOCK.replace(
            f'ai_standards_version = "{CURRENT_RELEASE_PIN}"',
    ...
    assert CURRENT_RELEASE_PIN in message
...
def test_sync_deploys_code_review_report_template_without_any_agents(tmp_path: Path) -> None:
    ...
    # The report template and the reporting reference are agent-agnostic: they
    # land even with no adapters declared.
    assert [result.status for result in results] == ["created", "created"]
    ...
    reference = project_root / ".ai-standards" / "references" / "code-review-reporting.md"
    assert reference.exists()
    reference_content = reference.read_text(encoding="utf-8")
    assert "Managed by ai-standards template:" in reference_content
    assert "reporting policy" in reference_content
