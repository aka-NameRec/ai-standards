...
def _init_demo_repo(tmp_path: Path) -> Path:
    ...
    (repo_root / "meta.toml").write_text(
        (
            "[release]\n"
            'version = "0.1.0"\n'
            'date = "2026-04-15"\n'
...
def test_save_release_updates_repo_metadata_and_agents(tmp_path: Path) -> None:
    ...
    meta = (repo_root / "meta.toml").read_text(encoding="utf-8")
    ...
    assert '[release]\nversion = "0.2.0"' in meta
    assert 'date = "2026-04-20"' in meta
    assert 'ai_standards_version = "0.2.0-2026-04-20"' in manifest
    assert "project_version" not in manifest
    assert 'ai_standards_version = "0.2.0-2026-04-20"' in template_manifest
    assert "project_version" not in template_manifest
    assert "Generated by ai-standards v0.2.0 release date: 2026-04-20" in agents
    assert "Project version:" not in agents
...
def test_save_command_runs_via_uv_console_script(tmp_path: Path) -> None:
    ...
    meta = (repo_root / "meta.toml").read_text(encoding="utf-8")
    assert '[release]\nversion = "1.1.0"' in meta
    assert 'date = "2026-04-16"' in meta
